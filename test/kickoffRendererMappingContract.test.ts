...
describe('Kickoff logical renderer mappings', () => {
  it('maps Apparel content keys to executable code in the Apparel frontend composition', () => {
    ...
    const recordsRoot = path.join(kickoffRoot, 'modules', `agora.${domain}`, 'data', 'sample-v001', 'content', 'records');
    const data = require(path.join(recordsRoot, `agora${title}RendererData.js`));
