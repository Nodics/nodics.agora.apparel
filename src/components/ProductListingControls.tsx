...
import type {
  AgoraListingExperience,
  AgoraListingFilterGroup,
} from "../cms/agoraHomeContent";

export type ProductListingLayout =
  | "list"
  | "grid-2"
  | "grid-3"
  | "grid-4"
...
type ProductFilterDrawerGroup =
  | AgoraListingFilterGroup
  | { readonly code: ProductFilterKey | "price"; readonly label: string };

type ProductListingToolbarProps = {
  readonly compact?: boolean;
  readonly activeFilterCount: number;
  readonly activeFilterLabels: readonly {
    readonly key: string;
    readonly label: string;
  }[];
...
const visibleLayoutOptions = function (configuration?: AgoraListingExperience) {
  const configured = configuration?.toolbar?.layoutOptions
    ...
    .map((option) => {
      const fallback = defaultLayoutOptions.find(
        (item) => item.code === option.code,
      );
      return {
        code: option.code as ProductListingLayout,
        label: option.label || fallback?.label || option.code,
        dots: fallback?.dots ?? [4, 4, 4, 4],
      };
    });
...
const visibleSortOptions = function (configuration?: AgoraListingExperience) {
  const configured = configuration?.toolbar?.sortOptions?.filter(
    (option) => option.code && option.label,
  );
  return configured?.length ? configured : defaultSortOptions;
};

const fallbackDrawerGroups = function (
  filterGroups: readonly ProductFilterOptionGroup[],
): readonly ProductFilterDrawerGroup[] {
  const groups = filterGroups.map((group) => ({
    code: group.key,
    label: group.label,
  }));
  const categoryIndex = groups.findIndex(
    (group) => group.code === "categories",
  );
  const priceGroup = { code: "price" as const, label: "Price" };
  if (categoryIndex < 0) return [priceGroup, ...groups];
  return [
    ...groups.slice(0, categoryIndex + 1),
    priceGroup,
    ...groups.slice(categoryIndex + 1),
  ];
};
...
export function ProductListingToolbar({
  ...
  return (
    <section
      ...
      aria-label={
        configuration?.toolbar?.ariaLabel ?? "Product listing controls"
      }
    >
      <div className="plp-primary-controls">
        ...
        >
          ...
          {configuration?.toolbar?.filterLabel ??
            configuration?.filterLabel ??
            "Filters"}
        ...
        <label className="plp-sale-inline">
          ...
          {configuration?.toolbar?.saleOnlyLabel ??
            configuration?.saleOnlyLabel ??
            "Shop sale items only"}
        </label>
        <div
          className="plp-layout-toggle"
          aria-label={
            configuration?.toolbar?.layoutAriaLabel ?? "Product layout"
          }
        >
          {visibleLayoutOptions(configuration)
            .filter(
              (option) =>
                !compact || option.code === "list" || option.code === "grid-2",
            )
            .map((option) => (
              <button
              ...
              >
                <span
                  className={`plp-density-icon plp-density-icon-${option.code}`}
                  aria-hidden="true"
                >
                  {option.dots.map((dot, index) => (
                    <i key={`${option.code}-${dot}-${index}`} />
                  ))}
                </span>
              </button>
            ))}
        ...
        <label>
          ...
          <select
            aria-label={
              configuration?.toolbar?.sortAriaLabel ?? "Sort products"
            }
            onChange={(event) => onSortChange(event.target.value)}
            value={sortCode}
          >
            {visibleSortOptions(configuration).map((option) => (
      ...
      {activeFilterLabels.length ? (
        <div
          className="plp-active-filters"
          aria-label={
            configuration?.toolbar?.activeFiltersAriaLabel ?? "Active filters"
          }
        >
          ...
          <button onClick={onClearFilters} type="button">
            {configuration?.toolbar?.clearAllLabel ?? "Clear all"}
          </button>
        </div>
      ) : null}
      {searchFacetsText ? (
        <p className="visually-hidden" aria-label={searchFacetsLabel}>
          {searchFacetsText}
        </p>
      ) : null}
    </section>
...
export function ProductFilterDrawer({
  ...
  const configuredGroups: readonly ProductFilterDrawerGroup[] = drawer?.groups
    ?.length
    ? drawer.groups
    : fallbackDrawerGroups(filterGroups);
  ...
  return createPortal(
    ...
    >
      <aside
        className="plp-filter-drawer"
        aria-label={drawer?.ariaLabel ?? "Product filters"}
        aria-modal="true"
        role="dialog"
      >
        <div className="plp-filter-drawer-header">
          <div>
            <h3>{drawer?.title ?? "Filters"}</h3>
          </div>
          <button
            aria-label={drawer?.closeLabel ?? "Close filters"}
            onClick={onClose}
            type="button"
          >
        ...
        <div className="plp-filter-drawer-body">
          {configuredGroups.map((configuredGroup) => {
            if (configuredGroup.code === "price") {
              return (
                <fieldset className="plp-filter-group" key="price">
                  <legend>
                    {configuredGroup.label || drawer?.priceLabel || "Price"}
                  </legend>
                  <div className="plp-price-rail" aria-hidden="true">
                    <span />
                    <span />
                  </div>
                  <div className="plp-price-inputs">
                    <label>
                      {drawer?.minPriceLabel ?? "Min"}{" "}
                      <input
                        inputMode="decimal"
                        onChange={(event) =>
                          onPriceChange("priceMin", event.target.value)
                        }
                        placeholder={drawer?.minPricePlaceholder ?? "0"}
                        type="text"
                        value={filters.priceMin}
                      />
                    </label>
                    <label>
                      {drawer?.maxPriceLabel ?? "Max"}{" "}
                      <input
                        inputMode="decimal"
                        onChange={(event) =>
                          onPriceChange("priceMax", event.target.value)
                        }
                        placeholder={drawer?.maxPricePlaceholder ?? "250"}
                        type="text"
            ...
            const group = groupByKey.get(
              configuredGroup.code as ProductFilterKey,
            );
        ...
        <div className="plp-filter-actions">
          <button className="secondary" onClick={onClear} type="button">
            {drawer?.resetLabel ?? "Reset filters"}
          </button>
          <button onClick={onApply} type="button">
            {drawer?.applyLabel ?? "Apply filters"}
          </button>
        </div>
      </aside>
    </div>,
